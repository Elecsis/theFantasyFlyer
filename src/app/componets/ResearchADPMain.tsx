    ...
    return (
        <div className='flex flex-col w-full bg-white gap-5 px-8' >
            ...
            {data[0].players.map((player: any)=>(
                <Link href={`/research/players/${player.PlayerID}`} className="flex flex-row w-full border border-lime-500 rounded-full items-center justify-between pl-5 " key={player.player_id} id="topList">
                    <div className="w-10 h-10 stroke-lime-400"><MissingPersonIcon /></div>
